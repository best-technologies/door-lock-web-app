...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${API_BASE_URL}${API_VERSION}/attendance${queryString ? `?${queryString}` : ""}`;
    ...
    console.log("\n📡 [ATTENDANCE API] Fetching attendance records from backend...");
    ...
    console.log("\n✅ [ATTENDANCE API] Backend Response Received:");
    console.log("📊 Status:", response.status, response.statusText);
    console.log("📥 Response Data:", JSON.stringify(responseData, null, 2));
    console.log("📏 Response Size:", JSON.stringify(responseData).length, "bytes");
    
    if (responseData.data?.data) {
      console.log("📋 Records Count:", responseData.data.data.length);
    ...
    console.error("\n❌ [ATTENDANCE API] Error fetching attendance records:");
